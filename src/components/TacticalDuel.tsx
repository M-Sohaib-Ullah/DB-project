...
export function TacticalDuel({ allHeroes }: TacticalDuelProps) {
  ...
  const renderCard = (c: GameCard, hidden?: boolean, isSelectable?: boolean, active?: boolean, isCpuCard?: boolean) => (
    ...
    >
      <div className="h-1/2 w-full relative">
        <img src={hidden ? 'https://picsum.photos/seed/hidden/200/300' : c.image_url} className={cn("w-full h-full object-contain object-center", hidden ? "blur-md" : "grayscale")} />
    ...
    return !c ? <div className="w-32 h-48 border border-white/5 rounded bg-white/[0.01] flex items-center justify-center text-[10px] text-white/20 uppercase tracking-widest">Waiting</div> : (
      ...
      >
        <div className="h-24 relative">
          <img src={isHidden ? 'https://picsum.photos/seed/hidden/200/300' : c.image_url} className={cn("w-full h-full object-contain object-center transition-all", isHidden ? "blur-md" : "grayscale opacity-80")} />
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 mb-8">
          {draftPool.map(hero => {
            ...
            return (
              ...
              >
                <div className="h-1/2 w-full relative">
                  <img src={hero.image_url} className="w-full h-full object-contain object-center grayscale" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black to-transparent" />
                  <span className="absolute top-1 left-1 text-[8px] bg-white/10 px-1 py-0.5 rounded font-mono uppercase tracking-tighter text-white/70">
                    {hero.hero_type}
                  </span>
          ...
          {[...Array(5)].map((_, i) => {
             ...
             return (
               <div key={i} className="w-12 h-16 border rounded bg-white/5 border-white/20 overflow-hidden relative">
                 {activeHand[i] && <img src={activeHand[i].image_url} className="w-full h-full object-contain object-center opacity-50 grayscale" />}
               </div>
             );
          })}
        </div>
  ...
  return (
    <div className="flex flex-col flex-1 h-[700px] bg-white/[0.02] border border-white/10 rounded-lg overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative my-4 border-y border-white/5 bg-[#050505] overflow-hidden">
         <div className="absolute top-4 left-1/2 -translate-x-1/2 text-[10px] uppercase tracking-widest text-white/60 font-mono bg-white/5 px-4 py-1 rounded z-20 whitespace-nowrap">{message}</div>
         
         {phase === 'GAME_OVER' ? (
           <div className="text-4xl font-serif italic text-white my-8 tracking-tighter">
           ...
           <div className="flex items-center gap-12 mt-6">
             ...
             <div className="flex flex-col items-center">
               <motion.span 
                  ...
                  animate={{ scale: 1, opacity: 1 }}
                  className="font-serif italic text-white/20 text-3xl mb-2"
               >
                 {phase === 'RESOLVING' ? <span className="text-rose-500 drop-shadow-[0_0_8px_rgba(244,63,94,0.8)]">CLASH</span> : 'VS'}
               </motion.span>
               {activeStat && <span className="text-[8px] uppercase tracking-widest text-emerald-400 font-mono border border-emerald-400/30 bg-emerald-400/10 px-2 py-0.5 rounded">{activeStat} CONTEST</span>}
             </div>
         ...
         <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-4 text-[10px] font-mono text-white/30 uppercase tracking-widest">
           Rounds Remaining: <span className="text-white bg-white/10 px-2 py-0.5 rounded">{roundsLeft}</span>
         </div>
      </div>
